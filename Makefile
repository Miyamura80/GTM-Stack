...
lint_links: ## Lint all markdown links using pytest-check-links
	...
	@find . -name "*.md" -not -path "./.venv/*" -not -path "*/node_modules/*" | xargs uv run pytest -p no:cov -o "addopts=" --check-links --check-links-ignore "http://localhost:.*" --check-links-ignore "https://claude.ai/.*"
